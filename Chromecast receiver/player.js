   ...
   */
      /*window.nolSDKInstance = null;
   			var nolParams = {
				...
				// Initialize BSDK
				console.log("ggInitialize - " + nolParams);
				window.nolSDKInstance = NOLCMB.getInstance("ChromeCastInstance");
				window.nolSDKInstance.ggInitialize(nolParams);*/

  window.nolSDKInstance = NOLBUNDLE.nlsQ("DHG163HR-BH45-JKY6-BKH7-67GJKY68GJK7", "ChromeCastInstance");
...
//Handling optout
	...
	window.customMessageBus.onMessage = function(event) {
		...
		var optout_flag = event.data.data;
		optOut(optout_flag,'secure-uat');
	};
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.onPlaying_ = function(event) {
	console.log('on playing',event);
  this.log_('onPlaying');
  ...
  this.setState_(sampleplayer.State.PLAYING, crossfade);
    window.videoTimer= setInterval(function(){
		...
		console.log('currentTime',Math.round(event.path[0].currentTime));
		},1000);
};
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.onPause_ = function(event) {
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.onEnded_ = function(event) {
...
sampleplayer.CastPlayer.prototype.onSeekStart_ = function() {
	console.log('on seek start',event);
  this.log_('onSeekStart');
  clearTimeout(this.seekingTimeoutId_);
  this.element_.classList.add('seeking');
  //window.nolSDKInstance.ggPM('setPlayheadPosition',event.path[0].currentTime);
};
...
sampleplayer.CastPlayer.prototype.onVisibilityChanged_ = function(event) {
	console.log('on visiibilty changed',event);
  ...
  this.load(new cast.receiver.MediaManager.LoadInfo(
	  ...
	  var metadata = {
            ...
            "segC": "CustomSegmentValueC",
			      "ottStatus":event.data.media.metadata.kGCKMetadataNlsKeyOptout,
            "ottDeviceID" : event.data.media.metadata.kGCKMetadataNlsKeyNUID
			};
      console.log("3 - " + metadata);
      window.nolSDKInstance.ggPM(3, metadata);
      if(["1",1,true].indexOf(metadata.ottStatus)>=0){
           var contentMetadataObject ={
                type: "content",
                ottStatus: metadata.ottStatus,
                ottType: "casting",
                ottDevice: "chromecast",
                ottDeviceName: "Google Chromecast",
                ottDeviceID: metadata.ottDeviceID,
                ottDeviceManufacturer: "Google",
                ottDeviceModel: "ChromeCastModel",
                ottDeviceVersion: "1.0.0", 
                ottOptOutStatus: true,
...
sampleplayer.getProtocolFunction_ = function(mediaInformation) {
	console.log('media informtion object',mediaInformation);
  var url = mediaInformation.contentId;
  var mediaMetadata=mediaInformation.metadata;
  console.log('IDFA is',mediaMetadata.kGCKMetadataNlsKeyIDFA);
  console.log('NUID is',mediaMetadata.kGCKMetadataNlsKeyNUID);
  console.log('OptOut is',mediaMetadata.kGCKMetadataNlsKeyOptout);
  
  optOut(mediaMetadata.kGCKMetadataNlsKeyOptout,'secure-uat');
