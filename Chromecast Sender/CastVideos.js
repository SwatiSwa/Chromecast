...
CastPlayer.prototype.initializeCastPlayer = function() {
    ...
    options.receiverApplicationId = /*chrome.cast.media.DEFAULT_MEDIA_RECEIVER_APP_ID*/'07C1872D';
    ...
    cast?cast.framework.CastContext.getInstance().setOptions(options):null;
...
CastPlayer.prototype.switchPlayer = function() {
    ...
    if (cast && cast.framework) {
        var contentMetadataObject ={
                ...
                //nol_uoo : nolSDKInstance.getOptOutStatus()
            };

        if (this.remotePlayer.isConnected) {
            contentMetadataObject.ottStatus = "1";
            nolSDKInstance.ggPM('updateott',contentMetadataObject);
            console.log("Called update OTT event with ottStatus : " + contentMetadataObject.ottStatus);
            this.setupRemotePlayer();
            return;
        }
    }
    contentMetadataObject.ottStatus = "0";
    nolSDKInstance.ggPM('updateott',contentMetadataObject);
    console.log("Called update OTT event with ottStatus : " + contentMetadataObject.ottStatus);
    this.setupLocalPlayer();
};
...
var PlayerHandler = function(castPlayer) {
    ...
    this.play = function() {
        ...
        window.videoTimer= setInterval(function(){
            window.nolSDKInstance.ggPM('play',Math.round(castPlayer.currentMediaTime));
            window.nolSDKInstance.ggPM('setPlayheadPosition',Math.round(castPlayer.currentMediaTime));
            console.log('currentTime',Math.round(castPlayer.currentMediaTime));
        },1000);
    ...
    this.pause = function() {
        ...
        this.target.pause();
        window.nolSDKInstance.ggPM('pause',Math.round(castPlayer.currentMediaTime));
        castPlayer.playerState = PLAYER_STATE.PAUSED;
    ...
    this.stop = function() {
        ...
        window.nolSDKInstance.ggPM('stop',Math.round(castPlayer.currentMediaTime));
    ...
    this.loaded = function() {
        ...
        var currentMediaInfo = castPlayer.mediaContents[castPlayer.currentMediaIndex];
        var metadata = {
            "type": "content",
            "dataSrc": "cms",
            "assetid": "vid-123",
...
/*    var contentMetadataObject ={
        ...
        ottDeviceModel: "ChromeCastModel",
        ottDeviceVersion: "1.0.0", 
        ottOptOutStatus: nolSDKInstance.getOptOutStatus()
    };
    nolSDKInstance.ggPM('updateott',contentMetadataObject);
    console.log("Called update OTT event" + contentMetadataObject.ottStatus);*/
    ...
    playerTarget.load = function (mediaIndex) {
        ...
        /*castSession.loadMedia(request).then(function(){
            ...
            var contentMetadataObject ={
                ...
                ottDevice: "chromecast",
                ottDeviceName: "Google Chromecast",
                ottDeviceID: olSDKInstance.getId(),
                ottDeviceManufacturer: "Google",
                ottDeviceModel: "ChromeCastModel",
                ottDeviceVersion: "1.0.0", 
                ottOptOutStatus: nolSDKInstance.getOptOutStatus()
...
CastPlayer.prototype.endPlayback = function() {
    ...
    window.nolSDKInstance.ggPM('end',Math.round(castPlayer.currentMediaTime));
